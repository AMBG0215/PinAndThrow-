    ...
    <div class="content">
      ...
      <div class="grid-2">
        ...
        <div class="panel" style="display:flex;flex-direction:column;">
          <div class="tab-bar">
            <a class="tab <?= $active_tab === 'all' ? 'active' : '' ?>" href="?tab=all">All <span
                class="count"><?= intval($counts['total'] ?? 0) ?></span></a>
            <a class="tab <?= $active_tab === 'pending' ? 'active' : '' ?>" href="?tab=pending">Pending <span
                class="count"><?= intval($tab_counts['pending'] ?? 0) ?></span></a>
            <a class="tab <?= $active_tab === 'inprogress' ? 'active' : '' ?>" href="?tab=inprogress">In-Progress <span
                class="count"><?= intval($tab_counts['inprogress'] ?? 0) ?></span></a>
            <a class="tab <?= $active_tab === 'resolved' ? 'active' : '' ?>" href="?tab=resolved">Resolved <span
                class="count"><?= intval($tab_counts['resolved'] ?? 0) ?></span></a>
            <a class="tab <?= $active_tab === 'rejected' ? 'active' : '' ?>" href="?tab=rejected">Rejected <span
                class="count"><?= intval($tab_counts['rejected'] ?? 0) ?></span></a>
          </div>
            ...
            <div style="width:55%;border-right:1px solid var(--border);">
              <div class="report-list" id="reportList">
                <?php if (empty($reports)): ?>
                  <div class="empty-state">No reports yet. <a href="report.php" style="color:var(--accent);">Report waste
                      now</a>.</div>
                <?php else: ?>
                  <?php foreach ($reports as $i => $r):
                    ...
                    <div class="report-row <?= $i === 0 ? 'selected' : '' ?>" onclick="selectReport(this)"
                      data-id="<?= intval($r['report_ID']) ?>" data-loc="<?= $loc ?>" data-date="<?= $date ?>"
                      data-desc="<?= $desc ?>" data-status="<?= htmlspecialchars($r['status']) ?>" data-img="<?= $img ?>"
              ...
              <div class="detail-body">
                ...
                <div class="detail-row">
                  ...
                  <span class="detail-val"
                    id="detailLoc"><?= !empty($reports) ? htmlspecialchars($reports[0]['locationName'] ?? '—') : '—' ?></span>
                </div>
                <div class="detail-row">
                  <span class="detail-key">Submitted</span>
                  <span class="detail-val mono"
                    id="detailDate"><?= !empty($reports) ? date('M d, Y · h:i A', strtotime($reports[0]['timestamp'])) : '—' ?></span>
                </div>
        ...
        <div class="panel" id="notifications">
          <div class="panel-header">
            <div class="panel-title">
              <div class="dot" style="background:var(--accent)"></div>Updates from Barangay
            </div>
              ...
              foreach ($notifications as $n):
                ...
                <div class="notif-item <?= $unread ? 'unread' : '' ?>" data-notif-id="<?= (int) $n['notification_ID'] ?>">
                  <div class="notif-icon">
                    <?= $n['status'] === 'resolved' ? '✅' : ($n['status'] === 'inprogress' ? '🔄' : '📋') ?></div>
                  <div class="notif-body">
                    <div class="notif-msg"><?= htmlspecialchars($n['message']) ?></div>
                    <div class="notif-meta">#RPT-<?= str_pad($n['report_ID'], 4, '0', STR_PAD_LEFT) ?> · <?= $time_str ?>
                    </div>
                  </div>
                </div>
              <?php endforeach; ?>
